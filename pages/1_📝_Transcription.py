        ...
        with st.spinner("Creating Troweb job..."):
            ...
            if result and "data" in result:
                for item in items:
                    ...
                    if file_name:
                        st.session_state.transcript_ids[file_name] = result["data"].get(
                            "_id"
                        )
    ...
    try:
        ...
        async with session.get(url) as response:
            if response.status == 200:
                ...
                raise Exception(f"Failed to download {s3_key}: HTTP {response.status}")
    except Exception as e:
        temp_file.close()
    ...
    try:
        async with aiohttp.ClientSession() as session:
            ...
            try:
                ...
                st.session_state.file_statuses[file_key]["transcription"] = "completed"
