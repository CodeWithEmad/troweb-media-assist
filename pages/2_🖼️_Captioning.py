        ...
        with st.spinner("Creating Troweb job..."):
            ...
            if result and "data" in result:
                for item in items:
                    ...
                    if file_name:
                        st.session_state.caption_ids[file_name] = result["data"].get(
                            "_id"
                        )
...
# Source selection
source = st.radio("Select Source", ["Upload Files", "Load from S3"], key="image_source")
                ...
                with col2:
                    with st.spinner("Generating caption..."):
                        try:
                            ...
                            processed_items.append(
                                {
                                    "title": os.path.splitext(os.path.basename(s3_key))[
                                        0
                                    ],
                                    "caption": caption,
                                    "url": url,
